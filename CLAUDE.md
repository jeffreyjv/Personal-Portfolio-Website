...
- **`section-tint`** is the surface band: a **translucent** (0.55) `--surface` fill that fades to transparent at both ends. Use it instead of `bg-surface` on a section. A flat opaque fill draws a hard horizontal line against the transparent sections either side, and since there's no snapping you rest on those boundaries constantly. The translucency is separately load-bearing: `PageAurora` is one fixed layer behind the whole document, and an opaque band punches a hole in it for the length of the section — reinstating the "every section is its own world" reading the continuous background exists to remove.
...
- **Reduced motion** is handled at two layers: `<MotionConfig reducedMotion="user">` in `App.jsx` covers all `variants`/`animate`/`layout`. It does *not* cover scroll-linked `useTransform`, infinite loops, or interval-driven text — those call `useReducedMotion()` and hard-disable (see `HeroSection`, `PageAurora`).
...
- **`src/components/Ambient.jsx`** owns *panel-local* ambient motion. **`AmbientOrbs`** is the slow drifting-wash layer, and after the `PageAurora` merge only **Contact** uses it — inside its rounded card, where the panel edge is the point. About's full-bleed band was removed: a section-scoped aurora arriving at one seam and masked off at the other is exactly what made About read as a separate world. **Don't add a section-scoped orb band back**; the page aurora is the background now, and a second one on top only fights it. Callers pass positions/colors/durations; the drift itself lives in the component so the two sections can't diverge. Durations must stay **non-harmonic** (About 23/29/37, Contact 19/25/31) or the orbs re-sync into one visible pulse, and only `transform`/`opacity` may animate — these are 400–600px `blur-3xl` surfaces and anything that repaints them is the most expensive thing on the page. Same `useIsDesktop()` gate as everything else; off, they render but hold still. The host `<section>` needs `relative isolate` — `section-tint` paints a real background on that element, and without a stacking context the `-z-10` orb layer disappears behind it. The overflow clip lives *inside* `AmbientOrbs`, never on the section: an `overflow: hidden` ancestor would make About's sticky column a scroll container and kill the stick.
- **`fadeEdges` is mandatory on anything full-bleed**, and off by default for panels. The wrapper's clip is a *hard* clip, so a layer that overhangs its section gets sheared flat into a hard horizontal line at the boundary — the exact seam `section-tint` exists to prevent. `fadeEdges` masks over the same 12%/88% stops `section-tint` uses, so the effect and the surface band arrive and leave together; keep the two in step if either changes. About's `AmbientSheen` passes it because `inset-y-0` makes the sheen exactly section-tall, so its ends land on the seam. Contact passes neither: it's a rounded card, where the panel edge is the point.
- **`src/components/PageAurora.jsx` is the background for the entire document, and there is exactly one of it.** It replaced `StarBackground` *and* a hero-only aurora, and that merge is the point: a background clipped to a section makes that section's boundaries visible, which is what had the page reading as a stack of separate slabs. It's `fixed`, so the blobs never scroll away and never restart — the content travels past a background that's always there. What changes with scroll is the arrangement: per-blob travel at unequal rates (-90 to -420px) for depth, an `hsla()` wash interpolating blue→violet→teal→blue for hue, and an intensity ramp that is loudest at the hero and settles to 0.7 for the body. **The intensity floor is never 0** — the moment the aurora fades out under the content, the sections go back to being slabs. It's much louder than `Ambient` (0.6 alpha vs 0.22, saturated hues, `mix-blend-multiply` / `dark:mix-blend-screen` so overlaps deepen in light and glow in dark) and still keeps every rule: transform/opacity only, non-harmonic durations (17/21/27/33/41s), desktop + reduced-motion gate, pointer parallax in motion values rather than state.
  - **`-z-10`, not `z-0`.** Several sections are `position: static` and would paint *underneath* a `z-0` fixed layer. Survivable at StarBackground's 0.06 alphas, not at these.
  - **Nothing above it may be opaque.** Negative-z descendants paint before block backgrounds, so `bg-background` on `Home`'s wrapper div covered the aurora completely — it's removed, and `body` supplies that background via canvas propagation instead. Don't put it back.
  - Blobs nest two elements: the outer owns scroll travel, the inner the idle drift loop. On one element the variant system claims `y` and the parallax silently does nothing.
  - The hero's `section-shell` has to stay on the *inner* div, or the hero caps at `--measure` and the background stops being full-bleed.
...
- `Home.jsx` uses `overflow-x-clip`, not `overflow-x-hidden` — the latter silently creates a scroll container and breaks `position: sticky` and `useScroll`. That wrapper also carries **no** `bg-background`; see `PageAurora`.
- Sections are driven by `useSectionScroll`, whose travel is deliberately small (22px / 0.99). Cranking it back up makes each section announce itself as a separate panel arriving, and the page reads as five things moving at five speeds instead of one thing scrolling.
